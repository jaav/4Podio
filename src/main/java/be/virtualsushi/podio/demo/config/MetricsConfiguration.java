	...
	public static class GraphiteRegistry implements EnvironmentAware {
		...
		private void init() {
			...
			if (graphiteEnabled) {
				...
				GraphiteReporter graphiteReporter = GraphiteReporter.forRegistry(metricRegistry).convertRatesTo(TimeUnit.SECONDS).convertDurationsTo(TimeUnit.MILLISECONDS).build(graphite);
